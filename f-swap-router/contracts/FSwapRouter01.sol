...
import './interfaces/IFSwapRouter01.sol';
import './interfaces/IERC20.sol';
import './interfaces/IWETH.sol';

contract FSwapRouter01 is IFSwapRouter01 {
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) private {
        for (uint i; i < path.length - 1; i++) {
            ...
            IFSwapPair(FSwapLibrary.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(path[0], msg.sender, FSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) public pure override returns (uint amountIn) {
        return FSwapLibrary.getAmountOut(amountOut, reserveIn, reserveOut);
    }
